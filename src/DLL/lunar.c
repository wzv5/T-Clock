...
void GetLunarStr(int year, int month, int day, wchar_t* buf)
{
	...
	const wchar_t* ChDay[] = { L"*",L"初一",L"初二",L"初三",L"初四",L"初五",
	...
	const wchar_t* ChMonth[] = { L"*",L"正",L"二",L"三",L"四",L"五",L"六",L"七",L"八",L"九",L"十",L"十一",L"腊" };
	...
	wcscat(buf, ChMonth[(LunarCalendarDay & 0x3C0) >> 6]);
	wcscat(buf, L"月");

	wcscat(buf, ChDay[LunarCalendarDay & 0x3F]);
}
